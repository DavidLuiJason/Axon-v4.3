...
/**
 * Unified Capability Contract definition for Color & Contrast Intelligence.
 * Lives directly alongside the feature implementation.
 */
export const colorIntelligenceCapability: CapabilityContract = {
  id: 'color_intelligence',
  name: 'Color & Contrast Intelligence',
  description: 'Analyzes color contrast, calculates WCAG 2.1 ratios, evaluates accessibility compliance, and converts color representations.',
  category: 'Design & Accessibility',
  commands: [
    {
      name: 'contrast',
      syntax: '/contrast <foreground> <background>',
      description: 'Calculates the WCAG 2.1 contrast ratio and rating between two colors.',
      isExplicitSlash: true,
      examples: ['/contrast #ffffff #000000', '/contrast #10b981 #09090b'],
    },
  ],
  intents: [
    {
      intent: 'check_contrast',
      description: 'Calculates the WCAG contrast ratio between two hex or rgb colors.',
      supportedPolicies: ['immediate'],
    },
    {
      intent: 'get_auto_contrast',
      description: 'Determines the optimal high-contrast foreground color for a background.',
      supportedPolicies: ['immediate'],
    },
  ],
  planning: {
  ...
  selfDescription: {
    whatItDoes: 'Calculates WCAG 2.1 color contrast ratios and provides accessibility ratings for text and background combinations.',
    whatItRequires: 'Foreground and background hex or rgb colors.',
    ...
    resultType: 'Contrast ratio and WCAG rating string',
  },
  verifyResult: (attempt, result) => {
    return { verified: Boolean(result?.success || result?.ratio || result?.optimal) };
  },
  execute: (intent, target, params) => {
    const fg = params?.foreground || params?.fg || '#ffffff';
    const bg = params?.background || params?.bg || target || '#000000';
    if (intent === 'get_auto_contrast') {
      const optimal = getAutoContrastColor(bg);
      return {
        success: true,
        executed: true,
        purpose: 'report_result',
        response: `Optimal high-contrast color for **${bg}** is **${optimal}**.`,
      };
    }
    const ratio = getContrastRatio(fg, bg);
    const quality = getContrastQuality(ratio);
    return {
      success: true,
      executed: true,
      purpose: 'report_result',
      response: `WCAG 2.1 Contrast Ratio between **${fg}** and **${bg}**: **${ratio.toFixed(2)}:1** (${quality.label}).`,
      metadata: { ratio, rating: quality.label },
    };
  },
};
