...
/**
 * Unified Capability Contract definition for Project Timeline & Activity Tracking.
 * Lives directly alongside the feature implementation.
 */
export const projectTimelineCapability: CapabilityContract = {
  id: 'project_timeline',
  name: 'Project Timeline & Activity Engine',
  description: 'Queries project milestones, development events, timeline records, and activity summaries.',
  category: 'Workspace',
  commands: [
    {
      name: 'timeline',
      syntax: '/timeline [project]',
      description: 'Queries activity timeline and milestones.',
      isExplicitSlash: true,
      examples: ['/timeline', '/timeline general'],
    },
  ],
  intents: [
    {
      intent: 'query_timeline',
      description: 'Retrieves chronological activity milestones and events for a project.',
  ...
  selfDescription: {
    whatItDoes: 'Tracks project activity history, creation milestones, notes authorship, and code execution runs.',
    whatItRequires: 'Optional project identifier.',
    whenAvailable: 'Always available offline.',
    whatMakesItUnavailable: 'Never unavailable.',
    canRunOffline: true,
    hasKnownAlternatives: true,
    hasSideEffects: false,
    requiresConfirmation: false,
    resultType: 'Timeline activity summary',
  },
  verifyResult: (attempt, result) => {
    return { verified: Boolean(result?.success) };
  },
  execute: (intent, target) => {
    const events = DEFAULT_PROJECT_ACTIVITIES.slice(0, 4);
    const summary = events.map((e) => `• [${e.dateString}] **${e.title}**: ${e.summary}`).join('\n');
    return {
      success: true,
      executed: true,
      purpose: 'report_result',
      response: `### Project Timeline Milestones\n${summary}`,
    };
  },
};
